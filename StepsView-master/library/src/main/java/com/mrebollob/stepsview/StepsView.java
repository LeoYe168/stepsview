...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
    ...
    public StepsView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.steps_view);
            thumbSize = ta.getDimension(R.styleable.steps_view_thumb_size, 100f);
            ta.recycle();
        }
    ...
    private void init() {
        ...
        if (thumbSize != 0) mStepsViewIndicator.setThumbSize(thumbSize);
    ...
    public StepsView setThumbSize(int size) {
        mStepsViewIndicator.setThumbSize(size);
        return this;
    }
